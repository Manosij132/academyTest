...
{
    public class TdcAdminPredicate : AbstractAdminPredicate
    {
        private readonly Expression<Func<Employee, bool>> rolePredicate;
        public TdcAdminPredicate(IAuthenticatedUserService authenticatedUserService) : base(authenticatedUserService)
        {
            rolePredicate = e => authUserRoles.Contains(e.Tdc.ToLower());
        }
        ...
        {
            return string.Format(FetchEmployeeFilteredStartesWithBase(startsWith), $"OR Tdc IN ('{string.Join("','", authUserRoles)}')");
        }
        ...
        {
            Expression<Func<Dashboard, bool>> dashboardPredicate = e => authUserRoles.Contains(e.Tdc.ToLower());
            return FetchDashboardBase(dashboardPredicate);
        }

        public override bool CanExtendEndDate(Employee employee)
        {
            return authUserRoles.Contains(employee.Tdc.ToLower());
        }
