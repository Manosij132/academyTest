...
{
    public class CommunityAdminPredicate : AbstractAdminPredicate
    {
        private readonly Expression<Func<Employee, bool>> rolePredicate;
        public CommunityAdminPredicate(IAuthenticatedUserService authenticatedUserService) : base(authenticatedUserService)
        {
            rolePredicate = e => authUserRoles.Contains(e.Community.ToLower());
        }
        ...
        {
            return string.Format(FetchEmployeeFilteredStartesWithBase(startsWith), $"AND Community IN ('{string.Join("','", authUserRoles)}')");
        }
        ...
        {
            Expression<Func<Dashboard, bool>> dashboardPredicate = e => authUserRoles.Contains(e.Community.ToLower());
            return FetchDashboardBase(dashboardPredicate);
        }

        public override bool CanExtendEndDate(Employee employee)
        {
            return authUserRoles.Contains(employee.Community.ToLower());
        }
        ...
        {
            // Query the database context to get distinct positions of employees belonging to the same community as the authenticated user,
            // ensuring that the comparison is case-insensitive.
            List<string> ecosystems = (from e in _dbContext.Employees
                                       where authUserRoles.Contains(e.Community.ToLower())
                                       select e.WorkingEcosystem).Distinct().ToList();

            // Define a predicate expression to filter EcosystemMaster entities that are active,
            // belong to the user's ecosystems (case-insensitive), and are marked as primary.
            Expression<Func<EcosystemMaster, bool>> pPredicate = e => e.IsActive && ecosystems.Contains(e.EcosystemName.ToLower()) && e.IsPrimary;

            // Query the database context to retrieve a list of EcosystemId for EcosystemMasters that are both primary and active,
            // and also belong to the user's ecosystems (case-insensitive).
            List<int> ids = [.. (from p in _dbContext.EcosystemMasters
                             where p.IsPrimary && p.IsActive
                             && ecosystems.Contains(p.EcosystemName.ToLower())
                             select p.EcosystemId)];

            // Define a second predicate expression to filter EcosystemMaster entities that are active,
            // are not marked as primary, and whose IDs are in the previously retrieved list of IDs.
            Expression<Func<EcosystemMaster, bool>> sPredicate = e => e.IsActive && ids.Contains(e.EcosystemId);
            //Expression<Func<EcosystemMaster, bool>> sPredicate = e => e.IsActive && !e.IsPrimary && ids.Contains(e.EcosystemId);
