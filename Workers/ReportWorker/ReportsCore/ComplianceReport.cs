...
{
    internal class ComplianceReport : IReport
    {
        ...
        private const string SHEET_NAME_COMPLIANCEREPORT = "ComplianceReport";
        ...
        public ComplianceReport(IServiceProvider serviceProvider, AppSetting appSetting)
        {
        ...
        private async Task Initialize(JobRequest jobRequest,List<string> emailList)
        {
            ...
            KeyValuePair<string, string> sheet = await _googleClient.CreateNewWorksheet($"ExportComplianceReport_{DateTime.UtcNow.ToString(_appSetting.DateTimeAsIdFormat)}");
            WorksheetId = sheet.Key;

            await _googleClient.AddNewEmptySheetAsync(WorksheetId, SHEET_NAME_COMPLIANCEREPORT);
        ...
        private async Task ExportDetailReport(int pageIndex, JobRequest jobRequest)
        {
            ...
            Tuple<int?, string, int>? SHEET_NAME_TRAININGS_lastrow = lastrowdata.FirstOrDefault(x => x.Item2 == SHEET_NAME_COMPLIANCEREPORT);
            range = (SHEET_NAME_TRAININGS_lastrow?.Item3 == 0) ? $"A{1}:Z" : $"A{SHEET_NAME_TRAININGS_lastrow?.Item3 + 1}:Z";
            await _googleClient.InsertFormulaByRange(WorksheetId, SHEET_NAME_COMPLIANCEREPORT, trainingsList, range);
