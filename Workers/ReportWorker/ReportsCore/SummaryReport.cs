...
namespace Academy.Workers.ReportWorker.ReportsCore
{
    internal class SummaryReport : IReport
    {
        ...
        private const string SHEET_NAME_SUMMARYREPORT = "SummaryReport";
        ...
        public SummaryReport(IServiceProvider serviceProvider, AppSetting appSetting)
        {
        ...
        {
            KeyValuePair<string, string> sheet = await _googleClient.CreateNewWorksheet($"ExportSummaryReport_{DateTime.UtcNow.ToString(_appSetting.DateTimeAsIdFormat)}");
            WorksheetId = sheet.Key;

            await _googleClient.AddNewEmptySheetAsync(WorksheetId, SHEET_NAME_SUMMARYREPORT);
        ...
        private async Task ExportDetailReport(int pageIndex, JobRequest jobRequest)
        {
            ...
            Tuple<int?, string, int>? SHEET_NAME_TRAININGS_lastrow = lastrowdata.FirstOrDefault(x => x.Item2 == SHEET_NAME_SUMMARYREPORT);
            range = (SHEET_NAME_TRAININGS_lastrow?.Item3 == 0) ? $"A{1}:Z" : $"A{SHEET_NAME_TRAININGS_lastrow?.Item3 + 1}:Z";
            await _googleClient.InsertFormulaByRange(WorksheetId, SHEET_NAME_SUMMARYREPORT, trainingsList, range);
