...
import { UpdateGXLeader } from "@shared/Interface/UpdateGxLeader.model";
import { LoaderService } from "@services/loader.service";
import { AcademyHttpService } from "@services/academy-http.service";

@Component({
  selector: 'mf-app-gx-leader-dialog',
  standalone: true,
  templateUrl: 'gx-leader-dialog.component.html',
  styleUrls: ['./gx-leader-dialog.component.css'],
  imports: [
...
export class GxLeaderDialogComponent implements OnInit {
  ...
  constructor(private LoaderService: LoaderService, private toastr: ToastrService,
    public dialogRef: MatDialogRef<GxLeaderDialogComponent>,
  ...
  ngOnInit() {
    ...
    this.data.leaders = (this.data.leaders ?? []).filter(
    ...
    this.allLeaders = this.data.leaders;
    this.data.leaders = this.allLeaders.filter((x: any)=> x.tdc.includes(this.propusedTdc) && x.communityName.includes(this.data.employee.community));
  ...
  onSelect(selectedEmp: any): void {
    if (this.proposedDojoGxLeader !== '' && this.proposedDojoGxLeader != null) {
      this.updateGxLeader.EmployeeId = selectedEmp.employeeId;
      this.updateGxLeader.DojoGxLeaderEmail = this.proposedDojoGxLeader;
      this.updateGxLeader.DojoGxGlobarEmail = selectedEmp.employeeEmail;
      this.updateGxLeader.GloberName = selectedEmp.employeeName;
      this.updateGxLeader.ProposedLeaderName = this.data.selectedLeader.employeeName;
      this.updateGxLeader.ProposedLeaderSeniority = this.data.selectedLeader.seniority;
      this.updateGxLeader.GloberSeniority = selectedEmp.seniority == null ? "" : selectedEmp.seniority;

      this.academyHttpService.UpdateGXLeader(this.updateGxLeader).subscribe({
        next: (data: any) => {
          //Success popup - shweta borse
          this.toastr.success(data?.message || TOASTER_MESSAGES.UPDATE_SUCCESS, 'Success');
          this.close(data);
        },
        error: (err) => {
          //Error popup - shweta borse
          this.toastr.error('Update unsuccessful. Please try again.', 'Error');
  ...
  onDelete(selectedEmp: any): void {
    if (this.proposedDojoGxLeader !== '' && this.proposedDojoGxLeader != null) {
      ...
      this.academyHttpService.DeleteGXLeader(this.updateGxLeader).subscribe({
        next: (data:any) => {
          //Success popup - shweta borse
          this.toastr.success(data?.message || TOASTER_MESSAGES.UPDATE_SUCCESS, 'Success');
          // this.close(data);
          this.data.selectedLeader = {};
          this.proposedDojoGxLeader = '';
          this.data.employee.proposedDojoGxLeader = '';
        },
        error: (err) => {
          //Error popup - shweta borse
          this.toastr.error('Update unsuccessful. Please try again.', 'Error');
  ...
  onOptionsLocationSelected(event: MatAutocompleteSelectedEvent) {
    ...
    this.data.leaders = this.allLeaders.filter((x: any)=> this.selectedTdc?.includes(x.tdc)  && this.selectedCommunity.includes(x.communityName));
    // this.leaderSelected = true;
    // this.showLeaderDetails = true;
    // if (!event.option.value) {
    //   this.leaderSelected = false;
  ...
  onCommunityChanged(option: MatOptionSelectionChange) {
      ...
      this.data.leaders = this.allLeaders.filter((x: any)=> this.selectedTdc.some((tdc: string) => x.tdc.includes(tdc)  && this.selectedCommunity.some((communityName: string) => x.communityName.includes(communityName))));
    ...
    onCountryChanged(option: MatOptionSelectionChange) {
      ...
      this.data.leaders = this.allLeaders.filter((x: any) =>
                                                  this.selectedTdc.some((tdc: string) => x.tdc.includes(tdc)) && this.selectedCommunity.some((communityName: string) => x.communityName.includes(communityName))
                                                );
  ...
  hasSelectedLeader() {
    return this.data.selectedLeader && Object.keys(this.data.selectedLeader).length > 0;
  }
